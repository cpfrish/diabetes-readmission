...
def create_target_distribution(
    ...
    df_target = Y.to_frame(name=target_name)
    
    chart = (
        alt.Chart(df_target)
        .mark_bar()
        .encode(
            x=alt.X(f"{target_name}:N", title=target_name.replace("_", " ").title()),
            y=alt.Y("count()", title="Count"),
            color=alt.Color(f"{target_name}:N", legend=None).scale(scheme="tableau20"),
        )
        .properties(
...
def generate_eda_report(
    ...
    if save_plots:
        fig_target.save(
            os.path.join(output_dir, "target_distribution.html")
        )
    display(fig_target)
