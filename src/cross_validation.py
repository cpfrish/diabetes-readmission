...
class SMOTEGroupKFoldCV:
    ...
    def __init__(
        ...
        Args:
            ...
            smote_sampling_strategy: SMOTE sampling strategy
                - "auto": resample minority class to match majority (recommended)
                - float: desired ratio of minority to majority class (0.5 = 50% of majority)
                - dict: target number for each class
            smote_k_neighbors: Number of nearest neighbors for SMOTE
            random_state: Random seed for reproducibility
            optimize_threshold: If True, find optimal probability threshold for F1 score
        """
    ...
    def fit_evaluate(
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups), 1):
            ...
            # Dynamically adjust k_neighbors based on minority class size
            minority_count = min(y_train_fold.value_counts().values)
            k_neighbors = min(self.smote_k_neighbors, minority_count - 1)
            
            if k_neighbors < 1:
                if verbose:
                    print(f"Warning: Minority class too small ({minority_count}). Skipping SMOTE.")
                ...
                try:
                    ...
                    if verbose:
                        print(f"After SMOTE (k_neighbors={k_neighbors}): {len(X_train_resampled)} samples")
            ...
            if self.optimize_threshold and y_pred_proba is not None:
                # Find optimal threshold that maximizes F1 score
                precision_arr, recall_arr, thresholds = precision_recall_curve(y_val_fold, y_pred_proba)
                ...
                self.optimal_thresholds_.append(optimal_threshold)
...
def evaluate_with_smote_group_kfold(
    ...
    Args:
        ...
        smote_sampling_strategy: SMOTE sampling strategy 
            - \"auto\": resample minority to match majority (recommended)
            - float: desired ratio of minority to majority class
        smote_k_neighbors: Number of neighbors for SMOTE
        random_state: Random seed
        verbose: Whether to print progress
        optimize_threshold: If True, find optimal probability threshold for F1
