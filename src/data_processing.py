...
def group_discharge_dispositions(df: pd.DataFrame) -> pd.DataFrame:
    """
    Group discharge dispositions into condensed categories.
    ...
    if "discharge_disposition_id" in df_copy.columns:
        mapping = {
            1: 1,  # Home
            2: 1,  # Home with home health service
            3: 2,  # Skilled nursing facility
            4: 2,  # Intermediate care facility
            5: 3,  # Another hospital
            6: 4,  # Hospice - home
            7: 5,  # Hospice - medical facility
            8: 6,  # Left against medical advice
            9: 7,  # Expired
        }
        df_copy["discharge_disposition_grouped"] = df_copy["discharge_disposition_id"].map(mapping)
        df_copy["discharge_disposition_grouped"] = df_copy["discharge_disposition_grouped"].fillna(8)  # Map unknowns to 'Other'
        df_copy["discharge_disposition_grouped"] = df_copy["discharge_disposition_grouped"].astype(int) # Ensure integer type
        df_copy.drop(columns=["discharge_disposition_id"], inplace=True)

    return df_copy
def group_admission_sources(df: pd.DataFrame) -> pd.DataFrame:
    """
    Group admission sources into condensed categories.
    ...
    if "admission_source_id" in df_copy.columns:
        mapping = {
            1: 1, #Referrals Category
            2: 1,
            3: 1,
            4: 2, # Transfers "Transfer from Hospital",
            5: 2,
            6: 2,
            7: 3,# ER
            8: 4,# Court/Law Enforcement
            9: 5,# Not Available
            10: 2,# Transfer from Home Health Care
            11: 6,# Readmission to Same Facility
            12: 2,# Transfer from Hospice
        }
        df_copy["admission_source_grouped"] = df_copy["admission_source_id"].map(mapping)
        df_copy["admission_source_grouped"] = df_copy["admission_source_grouped"].fillna(5)  # Map unknowns to 'Not Available'
        df_copy["admission_source_grouped"] = df_copy["admission_source_grouped"].astype(int) # Ensure integer type
        df_copy.drop(columns=["admission_source_id"], inplace=True)
...
def encode_categorical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 3. Group discharge dispositions (26 → 8 categories)
    df_encoded = group_discharge_dispositions(df_encoded)

    # 4. Group admission sources (17 → 6 categories)
    df_encoded = group_admission_sources(df_encoded)

    # 5. Handle medical_specialty (73 values → collapse to top 10 + Other)
    # 49% are missing (coded as "?"), so we group low-frequency specialties
    if "medical_specialty" in df_encoded.columns:
        top_specialties = df_encoded["medical_specialty"].value_counts().nlargest(10).index
    ...
    categorical_to_encode = [
        ...
        "admission_type_id",             # 8 values
        "discharge_disposition_grouped", # 8 values (after grouping)
        "admission_source_grouped",      # 6 values (after grouping)
        "medical_specialty",             # 11 values (after grouping)
